...
def getTopicAnalysis(plt, trendDict):
    ...
    # qury: str, sentiment: b64img, anal2: b64img, anal2: b64img,
    analysis = {'topic': trendDict['name']}
    ...
    labels = ['Positive ['+str(percentages[0])+'%]', 'Neutral [' +
              str(percentages[1])+'%]', 'Negative ['+str(percentages[2])+'%]']
    ...
    sortedFreqs = dict(
        sorted(wordFreqs.items(), key=itemgetter(1), reverse=True)[:20])
    # print(sortedFreqs)
    ...
    plt.xticks(range(len(sortedFreqs)), list(
        sortedFreqs.keys()), rotation=30, fontsize=8)
    ...
    # sns.barplot(df)
    # sns.barplot(x = df.values, y = df.index,data = df)
    topOrgsPeople = dict(sorted(topOrgsPeople.items(),
                         key=itemgetter(1), reverse=True)[:20])
    plt.style.use('fivethirtyeight')
    plt.bar(range(len(topOrgsPeople)), list(topOrgsPeople.values()))
    plt.xticks(range(len(topOrgsPeople)), list(
        topOrgsPeople.keys()), rotation=30, fontsize=8)
...
def doBigScrape(userInterests):
    ...
    for i in range(len(trendingDicts)):
        returnData['trendingAnalysis'].append(
            getTopicAnalysis(plt, trendingDicts[i]))
